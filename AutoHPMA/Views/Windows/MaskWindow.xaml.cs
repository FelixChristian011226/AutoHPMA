...
public partial class MaskWindow : Window
{
    ...
    public MaskWindow()
    {
        ...
        InitializeComponent();
        //this.InitializeDpiAwareness(); 删除
        ...
        try
        {
                ...
                foreach (var drawable in kv.Value)
                {
                    if (!drawable.IsEmpty)
                    {
                        drawingContext.DrawRectangle(Brushes.Transparent,
                            ...
                            //drawable.Rect);
                            new Pen(new SolidColorBrush(GetMediaColorFromDrawingColor(drawable.Pen.Color)), drawable.Pen.Width),
                            drawable.Rect);
                    }
                }
            }
            ...
            foreach (var kv in VisionContext.Instance().DrawContent.LineList)
            {
                foreach (var drawable in kv.Value)
                {
                    ...
                    drawingContext.DrawLine(new Pen(new SolidColorBrush(GetMediaColorFromDrawingColor(drawable.Pen.Color)), drawable.Pen.Width), drawable.P1, drawable.P2);
                }
            }
